...
$query->bindParam(':attendnce',$status,PDO::PARAM_STR);
...
<head>
    ...
    <title>Mark Attendance Auto</title>
        ...
        <div class="hk-pg-wrapper">
            ...
            <nav class="hk-breadcrumb" aria-label="breadcrumb">
                <ol class="breadcrumb breadcrumb-light bg-transparent">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Mark Attendance Auto</li>
                </ol>
            </nav>
            <!-- /Breadcrumb -->
...
<form name="check" method="post">        
                           ...
                           <div class="row">
                               <div class="col-sm">
                                       <center>
                                           <br>
                                           <input type="text" style="width:300px" name="matId">
                                           <input type="submit" class="btn-secondary" name="pass" value="pass">
...
if(isset($_POST['submit'])){
    ...
    if($query->rowCount() > 0)
    { ?>
      <p style="color: red;">  <?php $_SESSION["error"]="Today's Class attendance for this shift already marked"; 
      ?>
       <script> location.replace("mark-attendance-auto.php"); </script>
      </p>
    <?php } else {

    //check for student RFID
    if(isset($_POST["pass"])){

    $insert = "INSERT INTO tblrfid(studentId) VALUES ('".$_POST['matId']."')";
    mysqli_query($con,$insert);
    }

    $tempArray=array();
    $temp="SELECT * FROM tblrfid";
    $tempResult = mysqli_query($con,$temp);
    while($tempRow = mysqli_fetch_assoc($tempResult)){

        array_push($tempArray,$tempRow["studentId"]);

    }
    
    
    $sql="SELECT  tblstudents.id,studentName,studentClass,studentMarticulationId,tblparents.parentEmail as pemail from tblstudents join tblparents on tblparents.id=tblstudents.parentId where studentClass=:clss ";
    $query = $dbh -> prepare($sql);
    $query->bindParam(':clss',$clss,PDO::PARAM_STR);
    $query->execute();
    $results=$query->fetchAll(PDO::FETCH_OBJ);
    ...
    <td><?php echo htmlentities($result->studentMarticulationId);?></td>
    <td><input type="radio" name="attendance[<?php echo $result->id; ?>]" value="1" required="true" 
    <?php 
    if(isset($_POST["pass"]))
    { 

            if(in_array($result->studentMarticulationId , $tempArray))
                { 
                    echo "checked";
                }      
    ...
    ?> 
    >P 
        &nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" name="attendance[<?php echo $result->id; ?>]" value="2" required="true">L
        &nbsp;&nbsp;&nbsp;&nbsp;
        <input type="radio" name="attendance[<?php echo $result->id; ?>]" value="0" required="true"
        <?php 
    if(isset($_POST["pass"]))
    { 

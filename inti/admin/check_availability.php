...
// code for student Marticulation ID  availablity
if(!empty($_POST["crdata"])) {
 $id= $_POST['crdata'];
 $dta=explode("$",$id);
$clss=$dta[0];
$matId=$dta[1];
$sql ="SELECT studentUsername FROM  tblstudents WHERE studentMarticulationId=:matId and studentClass=:clss";
$query= $dbh -> prepare($sql);
...
if($query -> rowCount() > 0)
{
echo "<span style='color:red'> Marticulation ID already alloted to other student in this class .</span>";
 echo "<script>$('#submit').prop('disabled',true);</script>";
} else{	
echo "<script>$('#submit').prop('disabled',false);</script>";
}
